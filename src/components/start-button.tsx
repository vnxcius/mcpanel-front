...
export default function StartButton({ onStartInitiated }: StartButtonProps) {
  ...
  return (
    <button
      ...
      onClick={handleStart}
      className="bg-accent disabled:bg-accent/50 after:bg-accent hover:bg-accent/90 text-accent-foreground before:bg-accent disabled:before:bg-accent/50 relative flex w-full cursor-pointer items-center justify-center gap-1.5 border-2 border-black pt-2 pb-3 transition-colors before:absolute before:top-0 before:left-0 before:h-0.5 before:w-full before:brightness-125 after:absolute after:bottom-0 after:left-0 after:h-1 after:w-full after:brightness-50 hover:translate-y-px hover:after:h-[3px] disabled:text-neutral-400"
    >
